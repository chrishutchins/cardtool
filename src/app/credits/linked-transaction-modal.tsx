...
import { CreditUsage, CreditWithSlot, WalletCard } from "./credits-client";
...
export function LinkedTransactionModal({
  ...
  // Get the first transaction's date as the default override date
  const firstTxn = usage.user_credit_usage_transactions?.[0]?.user_plaid_transactions;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div 
        ref={modalRef}
        className="relative bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
          <h3 className="text-lg font-semibold text-white">
            {isClawback ? "Clawback Detected" : "Linked Transaction"}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="space-y-2">
            <div className="text-sm text-zinc-400">
              {isClawback ? "Clawback Transaction" : `Detected Transaction${transactions.length > 1 ? 's' : ''}`}
            </div>
            <div className="space-y-2">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-zinc-800 bg-zinc-900/50">
          ...
          >
            {isPending ? "Unlinking..." : "Unlink Credit"}
          </button>
          <button
            onClick={onClose}
