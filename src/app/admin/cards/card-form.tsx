...
export function CardForm({ action, issuers, currencies, userPrimaryCurrencyIds, defaultValues }: CardFormProps) {
  ...
  const [cardChargeType, setCardChargeType] = useState<"credit" | "charge" | "debit">(defaultValues?.card_charge_type ?? "credit");
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <select
            ...
            onChange={(e) => setCardChargeType(e.target.value as "credit" | "charge" | "debit")}
            className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-white focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
          ...
          <p className="mt-1 text-xs text-zinc-500">Charge cards have no preset spending limit. Debit cards are excluded from application rules.</p>
        </div>
        <div>
